...
if __name__ == '__main__':
    ...
    acc_rec = {"train": [], "valid": []}
    with open(r'D:\modelling/filestoSE/acc_classify_.json', 'rb') as file1:
        acc_rec = pickle.load(file1)
    with open(r'D:\modelling/filestoSE/loss_classify_.json', 'rb') as file2:
        loss_rec = pickle.load(file2)
    with open(r'D:\modelling/filestoSE/mat_classify_.json', 'rb') as file3:
    ...
    model.load_state_dict(torch.load('D:\modelling/checkpoint_classify/model23.pkl'))

    optimizer.load_state_dict(torch.load('D:\modelling/checkpoint_classify/optimizer23.pkl'))
    ...
    log_dir = os.path.join(config.current_file, "resultstoSE", time_str)
    ...
    for epoch in range(24, 30):
        ...
        if (epoch % 1 == 0) or (best_acc < max(acc_rec["valid"])):
            ...
            torch.save(model.state_dict(), "D:\modelling/checkpoint_classify/model{}.pkl".format(epoch))
            torch.save(optimizer.state_dict(),"D:\modelling/checkpoint_classify/optimizer{}.pkl".format(epoch))
...
#     3：在下次训练回合之前，读入之前保存的loss,acc，并且读入pkl文件
    with open(r'D:\modelling/filestoSE/acc_classify_.json','wb') as file1:
        pickle.dump(acc_rec,file=file1)  # wb 覆盖原先文件
    with open(r'D:\modelling/filestoSE/loss_classify_.json', 'wb') as file2:
        pickle.dump(loss_rec, file=file2)
    with open(r'D:\modelling/filestoSE/mat_classify_.json', 'wb') as file3:
